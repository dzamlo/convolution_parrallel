...
// Filename is the path to the kernel file and Kernel, a pointer to a kernel
// structure which will be filled
...
bool load_kernel(char *filename, kernel_t *kernel) {
    ...
    // Get the firt number in the first line. This is the kernel width/2 round
...
// src is the original image. dst is the future image with convolution. x and y
// are the coord of the pixel which will be convolved.
//
void convolve_pixel(img_t *src, img_t *dst, kernel_t kernel, int x, int y) {
    ...
    for (int i = -kernel.half_n; i <= kernel.half_n; i++) {
        for (int j = -kernel.half_n; j <= kernel.half_n; j++) {
            // Get the pixel value from source image (can be the pixel itself or
            // a neighbor)
...
// src is the original image. dst is the future image with convolution. Start_x
// and start_y is the coord of the first pixel to convolve.
//
